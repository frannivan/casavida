...
public class ImageController {
        ...
        try {
            ...
            if (originalFilename == null || originalFilename.isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(new MessageResponse("Nombre de archivo inválido"));
            }

            // Validate format
            if (!isValidImageFormat(originalFilename) && !"application/pdf".equals(file.getContentType())) {
                return ResponseEntity.badRequest()
                        .body(new MessageResponse("Formato no soportado. Formatos permitidos: JPEG, PNG, PDF, WebP"));
            }
            ...
            if ("application/pdf".equals(file.getContentType()) || extension.equalsIgnoreCase(".pdf")) {
                try (org.apache.pdfbox.pdmodel.PDDocument document = org.apache.pdfbox.pdmodel.PDDocument
                    ...
                    org.apache.pdfbox.rendering.PDFRenderer pdfRenderer = new org.apache.pdfbox.rendering.PDFRenderer(
                            document);
    ...
    public ResponseEntity<Resource> getFile(@PathVariable String filename) {
        try {
            ...
            throw new RuntimeException("Error detecting content type: " + e.getMessage());
        }
    }
}
