...
export class PolygonEditorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    initMap(): void {
        ...
        console.log("DEBUG: Map initialized with ID:", mapId);
        ...
        this.map.on('click', (e: L.LeafletMouseEvent) => {
            console.log("DEBUG: Map click event at:", e.latlng);
    ...
    handleMapClick(latlng: L.LatLng): void {
        ...
        if (this.activeTab === 'fraccionamiento' && this.selectedFraccionamiento?.poligonoDelimitador && this.currentPoints.length === 0) {
           console.log("DEBUG: Prompting to replace fraccionamiento polygon");
           if (!confirm("El fraccionamiento ya tiene un polígono. ¿Deseas comenzar uno nuevo?")) {
               console.log("DEBUG: Confirm cancelled");
               return;
           }
           console.log("DEBUG: Confirm accepted");
        ...
        if (this.activeTab === 'lotes' && this.selectedLote?.planoCoordinates && this.currentPoints.length === 0) {
             console.log("DEBUG: Prompting to replace lote polygon");
             if (!confirm("El lote ya tiene un polígono. ¿Deseas comenzar uno nuevo?")) {
                 console.log("DEBUG: Confirm cancelled");
                 return;
             }
             console.log("DEBUG: Confirm accepted");
        ...
        if (this.activeTab === 'lotes' && !this.selectedLote) {
            console.log("DEBUG: Map click but no lote selected");
            alert("Primero selecciona un Lote para editar.");
            return;
        }

        console.log("DEBUG: Adding point to currentPoints:", latlng);
